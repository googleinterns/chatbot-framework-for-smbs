...
// Handle messages from cloud sub using a pull subscriber
// Annotating this class with @Component will launch the pull subscriber on application start up
// NOT being used currently, added for future reference
public class PubSubPullSubscriber {
  ...
  PubSubPullSubscriber(
      @Value("${pubsubConfig.maxOutstandingElements}") final String maxOutstandingElements,
      @Value("${pubsubConfig.maxOutstandingBytes}") final String maxOutstandingBytes,
      @Value("${pubsubConfig.threadCount}") final String threadCount,
      final HangoutsAsyncService asyncService) throws InterruptedException,
      GeneralSecurityException, IOException {
    this.asyncService = asyncService;
    projectID = System.getenv("projectID");
    subscriptionID = System.getenv("subscriptionID");
    this.maxOutstandingElements = Long.parseLong(maxOutstandingElements);
    this.maxOutstandingBytes = Long.parseLong(maxOutstandingBytes);
    this.threadCount = Integer.parseInt(threadCount);
    launchSubscriber();
  }

  // launch the pull subscribers
  private void launchSubscriber() throws InterruptedException, IllegalArgumentException {
