...
public class HangoutsAsyncService extends AsyncService {
  ...
  // handler for a request from the chat client
  @Override
  @Async("asyncExecutor")
  public void chatServiceRequestHandler(final ChatServiceRequest chatServiceRequest)
    ...
    switch (chatServiceRequest.getRequestType()) {
      case ADDED:
        hangoutsMessageSender.sendMessageBasedOnMessageType(spaceID, THANKS_FOR_ADDING_MESSAGE,
  ...
  public void handleMessageEvent(final ChatServiceRequest chatServiceRequest) throws Exception {
    final String spaceID = chatServiceRequest.getSender().getChatClientGeneratedId();
    ...
    if (chatServiceRequest.getUserMessage().getAttachmentsCount() == 0) {
      ...
      if (currentContextList.contains(ChatServiceConstants.EXPECTING_IMAGES_CONTEXT)) {
        ...
        hangoutsMessageSender.sendMessageBasedOnMessageType(
            chatServiceRequest.getSender().getChatClientGeneratedId(), NOT_EXPECTING_IMAGE_MESSAGE,
            HANGOUTS_MESSAGE_TYPE.TEXT);
      }
    }
  }

  // send message to a user via their userID
  @Override
  @Async("asyncExecutor")
  ...
  // send a message to a user via the ID generated by the chat client for them
  @Override
  @Async("asyncExecutor")
  ...
  // handle an event trigger notification from the backend
  @Override
  @Async("asyncExecutor")
